...
public class PemesananTiketBioskop {
    ...
    static void cariID() {
        ...
        while (l <= r) {
            ...
            else if (daftarFilm[m].id < cari) l = m+1;
            else r = m-1;
        }
        System.out.println("Tidak ditemukan.");
    }
    ...
    static void statistikFilm() {
        ...
        for (int i = 0; i < jumlahData; i++) {
            ...
            if (daftarFilm[i].aktif) {
                ...
                jual += daftarFilm[i].tiketTerjual;
    ...
    static void inisialisasi30Data() {
        ...
        String[] nama = {"Avengers","Spider Man","Batman","Fast","John Wick"};
        String[] kat = {"Action","Action","Action","Action","Action"};
        String[] stat = {"Sedang Tayang","Sedang Tayang","Akan Tayang","Sedang Tayang","Akan Tayang"};
        int[] terjual = {90,80,70,60,50};
        int[] sedia = {10,20,30,40,50};

        for (int i = 0; i < 5; i++) {
            daftarFilm[jumlahData++] =
                    new Film(i+1,nama[i],kat[i],stat[i],terjual[i],sedia[i]);
        }
    }
}